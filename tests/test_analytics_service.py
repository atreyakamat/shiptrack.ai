...
def test_get_overview_stats(db, sample_shipment):
    stats = AnalyticsService.get_overview_stats()
    assert stats['total'] == 1
    assert stats['in_transit'] == 1

def test_get_delivery_rate(db, sample_shipment):
    stats = AnalyticsService.get_overview_stats()
...
def test_get_shipments_by_status(db, sample_shipment):
    counts = AnalyticsService.get_shipments_by_status()
